...
// product listings.
router.delete("/admin/servers/:id", requireAdmin, requireOperationClearance("delete_product"), async (req, res): Promise<void> => {
  const id = parseInt(String(req.params.id), 10);
  if (isNaN(id)) {
    res.status(400).json({ error: "Invalid server id" });
    return;
...
// protected dataset.
router.delete("/admin/device-sessions/:userId", requireAdmin, requireOperationClearance("delete_device_sessions"), async (req, res): Promise<void> => {
