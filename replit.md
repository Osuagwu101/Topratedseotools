...
- **Trust, customer support, reviews, social proof, and payment display**: managed from the admin panel's new **Trust & Support** tab and surfaced on the storefront without changing the existing design language.
  - `site_settings` exposes `businessEmail` (public/clickable toggles), `whatsappNumber`/`whatsappMessage`/`whatsappEnabled` (floating WhatsApp button), `paymentIconsEnabled`, and a customers-served counter (`customersServedBaseline`, `customersServedCountingMethod`, `customersServedManualCorrection`).
  - `testimonials` table stores name, avatar, text, optional rating, `published`, `isSample`, `permissionObtained`, and `sortOrder`. Admin CRUD + publish toggle + drag-style reorder + avatar upload. Public list shows only published testimonials on the homepage.
  - `reviews` table stores verified purchase reviews with status workflow (`pending`/`approved`/`rejected`/`hidden`), admin reply, and verified badge. Admin can filter by status/rating/product and moderate; product detail pages show approved reviews with a star breakdown.
  - `review_prompts` table is created automatically when an order is successfully activated (`activateOrderByReference`). A unique index on `(clerkUserId, orderId, productId)` prevents duplicates. The dashboard prompts the customer up to 3 times per eligible purchase (session-limited) to submit a review.
  - `payment_methods` table stores provider logos/text labels (e.g. Visa, Mastercard, Verve, Bank Transfer, OPay). Default set seeded on schema push. Admin can add/reorder/upload icons/disable; public footer shows enabled icons when the global toggle is on.
  - `customer_counter_audit` records every manual baseline/correction change for accountability.
  - All new endpoints live in `artifacts/api-server/src/routes/trust.ts` and are mounted in `routes/index.ts`.
